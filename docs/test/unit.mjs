...
t('every Google Books URL carries country=US (403-geolocation workaround)', async () => {
  const { gbUrl } = await import('../js/lookup.js');
  assert.ok(gbUrl('isbn:9780765326355').includes('country=US'));
  assert.ok(gbUrl('piranesi clarke', '&maxResults=5').includes('country=US'));
  assert.ok(gbUrl('piranesi clarke', '&maxResults=5').includes('maxResults=5'));
});
...
const { mergeGoodreadsInto, matchKey } = await import('../js/goodreads.js');

t('matches on ISBN when both have one', () =>
  assert.equal(matchKey({ isbn13: '9780765326355', title: 'Whatever' }),
    matchKey({ isbn13: '9780765326355', title: 'Different Title' })));
t('falls back to title and author', () =>
  assert.equal(matchKey({ title: 'Piranesi', authors: ['Susanna Clarke'] }),
    matchKey({ title: 'piranesi', authors: ['susanna clarke'] })));

t('backfills dates onto a book that has none', () => {
  const existing = { sessions: [{ id: '1', finish: null, finished: true, approx: false }], owned: null, genres: [] };
  const incoming = { sessions: [{ id: '2', finish: '2011-03-01', finished: true, approx: true }], owned: true, genres: ['Classics'] };
  const { book, changed } = mergeGoodreadsInto(existing, incoming);
  assert.equal(book.sessions[0].finish, '2011-03-01');
  assert.ok(changed.includes('dates'));
  assert.ok(changed.includes('owned'));
  assert.ok(changed.includes('genres'));
});

t('never overwrites dates you already have', () => {
  const existing = { sessions: [{ id: '1', finish: '2015-12-25', finished: true }] };
  const incoming = { sessions: [{ id: '2', finish: '2011-03-01', finished: true, approx: true }] };
  const { book, changed } = mergeGoodreadsInto(existing, incoming);
  assert.equal(book.sessions[0].finish, '2015-12-25');
  assert.ok(!changed.includes('dates'));
});

t('never overwrites a rating or review you changed', () => {
  const existing = { rating: 3, review: 'my own words', sessions: [] };
  const incoming = { rating: 5, review: 'goodreads text', sessions: [] };
  const { book, changed } = mergeGoodreadsInto(existing, incoming);
  assert.equal(book.rating, 3);
  assert.equal(book.review, 'my own words');
  assert.equal(changed.length, 0);
});

t('fills a rating only when there is none', () => {
  const { book, changed } = mergeGoodreadsInto({ rating: null, sessions: [] }, { rating: 4, sessions: [] });
  assert.equal(book.rating, 4);
  assert.ok(changed.includes('rating'));
});

t('leaves status alone — that is yours to curate', () => {
  const { book } = mergeGoodreadsInto({ status: 'dnf', sessions: [] }, { status: 'read', sessions: [] });
  assert.equal(book.status, 'dnf');
});

t('reports nothing changed when the book is already complete', () => {
  const full = { rating: 4, review: 'x', genres: ['Fantasy'], year: 2010, owned: true,
    sessions: [{ id: '1', finish: '2019-04-23' }] };
  const { changed } = mergeGoodreadsInto(full, { rating: 5, genres: ['Horror'], year: 1999, owned: false,
    sessions: [{ id: '2', finish: '2001-01-01' }] });
  assert.equal(changed.length, 0);
});
