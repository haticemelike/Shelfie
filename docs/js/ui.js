 ...
 *
 * Note: there is deliberately no innerHTML escape hatch here. Book titles and
 * descriptions come from third-party APIs, and every one of them is added with
 * createTextNode below, so nothing fetched can ever execute as markup.
 */
...
/* A null root means "the view isn't on screen", not a crash. */
export const $ = (sel, root = document) => (root ? root.querySelector(sel) : null);
export const $$ = (sel, root = document) => (root ? Array.from(root.querySelectorAll(sel)) : []);
