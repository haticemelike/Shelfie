...
import {
  ...
  cooldownRemaining, clearCooldown, probeServices, pacingReport,
  exportCooldowns, importCooldowns, setGoogleCredentials, hasGoogleKey,
} from './lookup.js';
    ...
    h('div', { class: 'settings' },
      ...
      h('section', { class: 'card' },
        ...
        missing.length
          ...
          : null,
        h('button', {
          class: 'btn btn--ghost btn--block', type: 'button', id: 'probe-btn',
          onclick: (e) => runProbe(e.currentTarget),
        }, 'Test the book services'),
        h('pre', { class: 'probe', id: 'probe-out', hidden: true })
      ),
      ...
      h('section', { class: 'card card--quiet' },
        h('h3', { class: 'card__title' }, 'Google Books key (optional)'),
        h('p', { class: 'muted tiny' },
          'Without a key, Google shares one small allowance between everyone on your '
          + 'network, which is what causes “too many requests” on a big fetch. A free '
          + 'key gives you 1,000 lookups a day of your own. Get one at '
          + 'console.cloud.google.com → enable the Books API → Create credentials → API key.'),
        h('p', { class: 'muted tiny' },
          'It is stored only on this phone and never goes into your GitHub repo — '
          + 'that repo is public, and a key committed there is a key anyone can spend.'),
        h('div', { class: 'inline-form' },
          h('input', {
            class: 'input', type: 'text', id: 'gkey', placeholder: 'AIza…',
        ...
        h('p', { class: 'muted tiny', id: 'gkey-state' }, '')
      ),
...
const KEY_META = 'googleApiKey';

/** Restore the key (and cooldowns) into lookup.js at startup. */
export async function loadNetworkSettings() {
  try {
    const key = await db.getMeta(KEY_META, '');
    let id = await db.getMeta('quotaUserId', '');
    if (!id) {
      // A random local id, not anything about you. Only sent alongside a key,
      // where it keeps one device from spending a whole project quota.
      id = 'u' + Math.random().toString(36).slice(2, 12);
      await db.setMeta('quotaUserId', id);
    }
    setGoogleCredentials({ key, user: id });
    importCooldowns(await db.getMeta('apiCooldowns', {}));
  } catch (_) { /* first run, or storage unavailable */ }
}

async function saveGoogleKey() {
  const field = document.getElementById('gkey');
  const key = field.value.trim();
  await db.setMeta(KEY_META, key);
  const id = await db.getMeta('quotaUserId', '');
  setGoogleCredentials({ key, user: id });
  field.value = '';
  toast(key ? 'Key saved on this phone.' : 'Key removed.');
  showGoogleKey();
}

async function showGoogleKey() {
  const line = document.getElementById('gkey-state');
  if (!line) return;
  const key = await db.getMeta(KEY_META, '');
  if (!key) {
    line.textContent = 'No key set — using the shared anonymous allowance.';
    return;
  }
  // Never render the key itself; a screenshot of Settings shouldn't leak it.
  line.textContent = `A key is saved (ending …${key.slice(-4)}). Clear the box and press Save to remove it.`;
}
...
/**
 * One request to each service against a book everyone has, so a bad run can be
...
async function runProbe(btn) {
  ...
  out.hidden = false;
  out.textContent = 'Asking each service about a book they definitely have…';

  const LABEL = {
    openLibraryData: 'Open Library — book details',
    openLibraryCoverDownload: 'Open Library — cover download',
    openLibraryCoverDisplay: 'Open Library — cover as an image',
    googleBooksData: 'Google Books — book details',
    googleCoverDisplay: 'Google Books — cover as an image',
  };
  ...
  try {
    ...
    const lines = Object.entries(result)
      .filter(([k]) => LABEL[k])
      .map(([k, v]) => `${v === 'ok' ? '✓' : '✗'}  ${LABEL[k]}: ${v}`);

    if (result.throttledDuringProbe) {
      lines.push('', `⚠ Turned away ${result.throttledDuringProbe} times — you are being rate limited.`,
        'Wait ten minutes or so and everything should work again.');
    } else if (result.networkErrorsDuringProbe) {
      lines.push('', `⚠ ${result.networkErrorsDuringProbe} requests could not reach the service at all.`,
        'That is usually no connection, or wifi that wants you to sign in.');
    } else if (lines.every((l) => l.startsWith('✓'))) {
      lines.push('', 'Everything is answering. Books still without covers genuinely have none '
        + 'under that edition — try ⋯ → Refresh on one, or photograph the cover.');
    }
    out.textContent = lines.join('\n');
  } catch (e) {
    out.textContent = 'The test itself failed: ' + (e.message || e);
  }

  btn.disabled = false;
  btn.textContent = 'Test the book services';
}

/**
 * Google is deliberately paced to about one request a second, and most books
 * that still need artwork end up asking it. Estimate honestly rather than
 * cheerfully — a wrong "under a minute" is how the last version looked broken.
 */
function etaFor(n) {
  const seconds = Math.round(n * (hasGoogleKey() ? 0.6 : 1.1));
...
async function enrich(list, btn) {
  ...
  resetNetStats();
  const counts = { done: 0, updated: 0, notFound: 0, failed: 0 };
  let gaveUp = false;
  ...
  const paint = () => {
    ...
    status.textContent =
      ...
      + (counts.failed ? ` · ${counts.failed} couldn’t be checked` : '')
      + (waiting > 500 ? ` · easing off for ${Math.ceil(waiting / 1000)}s` : '');
    if (waiting > 500) {
      const slow = Object.entries(pacingReport())
        .filter(([, v]) => v.waitingMs > 500)
        .map(([host]) => host.replace('www.googleapis.com', 'Google').replace(/\.?openlibrary\.org/, 'Open Library'));
      if (slow.length) status.textContent += ` (${[...new Set(slow)].join(', ')})`;
    }
  };
  paint();
  ...
  await runPool(list, async (book) => {
    ...
    // Nothing succeeding while books pile up unchecked: keep going is pointless.
    if (counts.updated === 0 && counts.failed >= 15) {
      clearCooldown();
      enrichAbort = true;
    }
    paint();
  ...
  if (gaveUp) {
  ...
  } else if (counts.updated === 0 && counts.failed >= 15) {
    status.textContent = `Stopped: ${counts.failed} books in a row could not be checked and `
      + 'nothing succeeded. The services are refusing requests right now — most likely because '
      + 'of the earlier runs. Nothing is wrong with your books. Use “Test the book services” '
      + 'below to confirm, then try again in ten or fifteen minutes.';
    ...
    status.textContent = `Done. ${counts.updated} updated`
      + (counts.notFound ? `, ${counts.notFound} genuinely had no match` : '')
      + (counts.failed
        ? `, ${counts.failed} could not be checked because the services were busy — `
          + 'run it again and those will be retried.'
        : '.');
  }

  db.setMeta('apiCooldowns', exportCooldowns()).catch(() => {});
  releaseAwake(wake);
  btn.dataset.running = '';
...
/**
 * Returns 'updated' | 'notfound' | 'failed' | 'skip'.
 *
 * 'failed' matters: a book whose requests were throttled or errored has NOT
 * been shown to lack a cover. Counting those as "no match" is what made a
 * rate-limited run look like a library full of obscure books.
 */
async function enrichOne(book) {
  const current = store.get(book.id);
  if (!current) return 'skip';

  // Per-book tallies. The global counters can't be used here: three books run
  // at once, so one throttled book would mark its neighbours blocked too.
  const ctx = {};
  const wasBlocked = () => (ctx.rateLimited || 0) > 0 || (ctx.networkErrors || 0) > 0;
  ...
  if (needMeta) {
    found = isbn ? await lookupIsbn(isbn, ctx) : null;
    if (!found) {
      const query = [current.title, (current.authors || [])[0]].filter(Boolean).join(' ');
      const hits = query ? await searchBooks(query, { limit: 3 }, ctx) : [];
  ...
  if (needCover) {
    const cover = await findCover(patch, found, ctx);
    if (cover) {
  ...
  return wasBlocked() ? 'failed' : 'notfound';
}
