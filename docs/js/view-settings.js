...
async function enrich(list, btn) {
  ...
  const paint = () => {
    ...
    status.textContent =
      ...
      + (counts.failed ? ` · ${counts.failed} blocked` : '')
...
async function enrichOne(book) {
  ...
  const before = { rateLimited: netStats.rateLimited, networkErrors: netStats.networkErrors };
  const wasBlocked = () => netStats.rateLimited > before.rateLimited
    || netStats.networkErrors > before.networkErrors;
