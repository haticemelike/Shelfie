...
function statusSection(book, rerender) {
  return h('section', { class: 'card' },
    ...
    // Duplicates are common — a spare, a gift copy, a different edition you
    // kept. One entry with a count beats two identical books on the shelf.
    book.owned === false ? null : h('div', { class: 'ownrow' },
      h('span', { class: 'field__label' }, 'How many copies?'),
      h('div', { class: 'stepper' },
        h('button', {
          class: 'stepper__btn', type: 'button', 'aria-label': 'One fewer copy',
          disabled: (book.copies || 1) <= 1,
          onclick: () => setCopies(book, (book.copies || 1) - 1, rerender),
        }, '−'),
        h('span', { class: 'stepper__value' }, String(book.copies || 1)),
        h('button', {
          class: 'stepper__btn', type: 'button', 'aria-label': 'One more copy',
          onclick: () => setCopies(book, (book.copies || 1) + 1, rerender),
        }, '+')
      )
    )
...
async function setCopies(book, copies, rerender) {
  const n = Math.max(1, Math.min(99, copies));
  await store.save({ ...store.get(book.id), copies: n, owned: n > 1 ? true : book.owned });
  rerender();
}
