...
import { fetchCoverBlob, lookupIsbn } from './lookup.js';
import { renderResults } from './view-library.js';
...
function sessionRow(book, s, rerender) {
  let label = '';
  let cls = '';
  if (s.finish) { label = `Finished ${fmtDate(s.finish)}`; cls = 'ok'; }
  else if (s.finished) { label = 'Finished — date unknown'; cls = 'ok'; }
  else if (s.dnfAt) { label = `Set aside ${fmtDate(s.dnfAt)}`; cls = 'dnf'; }
  else if (s.start) { label = `Started ${fmtDate(s.start)}`; cls = 'open'; }
  else label = 'Undated';
...
function moreActions(book, rerender) {
  const body = h('div', { class: 'menu' },
    menuItem('✎', 'Edit details', () => editSheet(book, rerender)),
    menuItem('⭯', 'Refresh info from the internet', () => refetch(book, rerender)),
    menuItem('🖼', 'Replace cover', () => replaceCover(book, rerender)),
    menuItem('🗑', 'Delete book', async () => {
...
async function refetch(book, rerender) {
  ...
  if (!merged.hasCover && found.coverUrl) {
    const blob = await fetchCoverBlob(found.coverUrl);
    if (blob) await store.setCover(merged.id, blob);
  }
  toast('Updated');
  rerender();
}
...
function replaceCover(book, rerender) {
  const input = h('input', { type: 'file', accept: 'image/*', capture: 'environment', class: 'input' });
  const body = h('div', { class: 'form' },
    h('p', { class: 'muted' }, 'Take a photo of the cover, or choose an image from your library.'),
    input
  );
    ...
    actions: [
      { label: 'Cancel' },
      {
        ...
        onClick: async () => {
          ...
          if (!file) { toast('Pick an image first.', { error: true }); return true; }
          const blob = await shrinkImage(file, 700);
          await store.setCover(book.id, blob);
          toast('Cover updated');
          rerender();
          renderResults();
        },
