...
async function fromOpenLibrary(isbn) {
  ...
  // The work record usually has the richest subject list.
  let workSubjects = [];
  let description = '';
...
export async function findCover(book, found = null) {
  const isbns = [book.isbn13, book.isbn10, found && found.isbn13, found && found.isbn10]
    .filter(Boolean);

  // 1. Open Library by ISBN — CORS-friendly, so we get a real offline copy.
  for (const isbn of [...new Set(isbns)]) {
    const blob = await fetchCoverBlob(coverUrlForIsbn(isbn));
    if (blob) return { blob, url: coverUrlForIsbn(isbn) };
  }

  // 2. Whatever the metadata lookup turned up.
  if (found && found.coverUrl) {
    const blob = await fetchCoverBlob(found.coverUrl);
    if (blob) return { blob, url: found.coverUrl };
    for (const variant of googleCoverVariants(found.coverUrl)) {
      if (await verifyImageUrl(variant)) return { blob: null, url: variant };
    }
  }

  // 3. Other editions of the same book. A specific printing often has no cover
  //    on file while another edition of the same work does.
  const query = [book.title, (book.authors || [])[0]].filter(Boolean).join(' ');
  if (!query) return null;

  const hits = await searchBooks(query, { limit: 5 });
  for (const hit of hits) {
    if (!plausibleMatch(book, hit) || !hit.coverUrl) continue;
    const blob = await fetchCoverBlob(hit.coverUrl);
    if (blob) return { blob, url: hit.coverUrl };
    for (const variant of googleCoverVariants(hit.coverUrl)) {
      if (await verifyImageUrl(variant)) return { blob: null, url: variant };
    }
  }
